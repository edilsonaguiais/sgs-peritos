...
#
"""Utilitarios de data e constantes regionais."""
...
BRAZILIAN_STATES = []
for _state in BRAZILIAN_REGIONS.values():
    BRAZILIAN_STATES.extend(_state)
...
class Date:
    def __init__(self, d: DateInput, format: str = "%Y-%m-%d") -> None:
        if isinstance(d, str):
            ...
            raise ValueError(f"Tipo de data nao suportado: {type(d).__name__}")
        self.date: date = d
